        ...
        <div class="box">
          <div class="box_border">
            ...
            <div class="box_center">
              <form action="admin/adminProduct/saveDetailed.json"  name="savePolicyInfoForm" id="savePolicyInfoForm"  method="post">
               ...
               <table id="table" class="form_table pt15 pb15" width="100%" border="0" cellpadding="0" cellspacing="0">
                ...
                <c:forEach var="salesList" items="${salesList}">
                 <tr>
                  ...
                  <td class="td_right" width="20%">
                                                                  属性名称：<input type="text" value="${salesList.ATTRIBUTE_TYPE }" id="ATTRIBUTE_TYPE" name="ATTRIBUTE_TYPE" class="input-text lh30" size="15">
                  </td>
                  <td align="center" width="20%">
                                                                     属性值:<input type="text" value="${salesList.SALES_ATTRIBUTE }" id="SALES_ATTRIBUTE" name="SALES_ATTRIBUTE" class="input-text lh30" size="15">
                  </td>
                   <td align="center" width="20%"> 
                                                                     商家编码:<input type="text" value="${salesList.MERCHANT_CODE }" id="MERCHANT_CODE" name="MERCHANT_CODE" class="input-text lh30" size="15">
                  </td>
                  <td> 
                                                                   附属价格:<input type="text" value="${salesList.SUBSIDIARY_PRICE }" id="SUBSIDIARY_PRICE" value=0 name="SUBSIDIARY_PRICE" class="input-text lh30" size="10">元
                  </td>
                </tr>
                </c:forEach>
                </c:when>
                <c:otherwise>
                  ...
                  <td class="td_right" width="20%">
                                                                  属性名称：<input type="text" value="" id="ATTRIBUTE_TYPE" name="ATTRIBUTE_TYPE" class="input-text lh30" size="15">
                  </td>
                  <td align="center" width="20%">
               ...
               <TR style="background: url('images/box_top.png') 0px 0px repeat-x;">
                ...
                <c:choose>
                <c:when test="${!empty airlinesList}">
                <c:forEach var="airlinesList" items="${airlinesList }">
                <tr>
                <td>&nbsp;</td>
                <td class="td_right">
                  <input type="text" value="${airlinesList.INSURE_ALLOWED }" id="INSURE_ALLOWED" name="INSURE_ALLOWED" class="input-text lh30" size="30">
                </td>
                <td align="center"><input type="text" value="${airlinesList.INFO }" id="INSURE_INFO" name="INSURE_INFO" class="input-text lh30" size="30"></td>
                <td ><input type="text" value="${airlinesList.GUARANTEE_AMOUNT }" id="GUARANTEE_AMOUNT" name="GUARANTEE_AMOUNT" class="input-text lh30" size="30"></td>
                </tr>
                </c:forEach>
                </c:when>
                ...
                <c:when test="${!empty policyList}">
                     <% String[] a = {"通讯地址","姓名","电子邮件","证件号码","出生日期","性别","常住地区","固定电话","手机号码","地址","邮编"};
                        String[] b = new String[a.length];
                        int i=0;
                      %>
                     <c:forEach var="policyList" items="${policyList}">
                        <c:set var="check" value="${policyList.POLICY_INFO}" scope="request"/> 
	                       <tr>
			                <td>&nbsp;</td>
			                <td>
			                  <input type="checkbox" name="POLICY_INFO" checked value="${policyList.POLICY_INFO}" >  ${policyList.POLICY_INFO}
			                </td>
			                <td align="center"><input type="text" value="${policyList.INFO}" id="INFO" name="INFO" class="input-text lh30" size="30"></td>
			                <td ><input id="spinner<%=k %>" value="${policyList.INFO_LEVEL}" name="INFO_LEVEL" size=8 readonly></td>
			               </tr>
			             <%
			               b[i] = request.getAttribute("check").toString();
			               i++;
			               k++;
			              %>
                     </c:forEach>
                      ...
                      if(null != check || check.length == 0){
                          for(int j=0;j<check.length;j++){
                           if(check[j] != null){
			                ...
			                <td>&nbsp;</td>
			                <td>
			                  <input type="checkbox" name="POLICY_INFO" value="<%=check[j] %>" >  <%=check[j] %>
			                </td>
			                <td align="center"><input type="text" id="INFO" name="INFO" class="input-text lh30" size="30"></td>
			                <td ><input id="spinner<%=k %>" value=5 name="INFO_LEVEL" size=8 readonly></td>
			                </tr>
                             <%
                             k++;
                             }
                ...
                <td>&nbsp;</td>
                <td>
                  <input type="checkbox" name="POLICY_INFO" value="固定电话">  固定电话
                </td>
                <td align="center"><input type="text" id="INFO" name="INFO" class="input-text lh30" size="30"></td>
                ...
                <c:choose>
                <c:when test="${!empty policyList1}">  
                        <% String[] a = {"通讯地址","姓名","电子邮件","证件号码","出生日期","性别","常住地区","固定电话","手机号码","地址","邮编"};
                           String[] b = new String[a.length];
                           int i=0;
                      %>
                     <c:forEach var="policyList" items="${policyList1}">
                        <c:set var="check1" value="${policyList.POLICY_INFO}" scope="request"/> 
	                       <tr>
			                <td>&nbsp;</td>
			                <td>
			                  <input type="checkbox" name="POLICY_INFO1" checked value="${policyList.POLICY_INFO}" >  ${policyList.POLICY_INFO}
			                </td>
			                <td align="center"><input type="text" value="${policyList.INFO}" id="INFO" name="INFO1" class="input-text lh30" size="30"></td>
			                <td ><input id="spinner<%=k %>" value="${policyList.INFO_LEVEL}" name="INFO_LEVEL1" size=8 readonly></td>
			               </tr>
			               
			           <% b[i] = request.getAttribute("check1").toString();
			              i++;
			              k++;
			           %>
                     </c:forEach>
                     <% String[] check = UtilString.minus(a,b);
                      if(null != check || check.length == 0){
                          for(int j=0;j<check.length;j++){
			                ...
			                <td>&nbsp;</td>
			                <td>
			                  <input type="checkbox" name="POLICY_INFO1" value="<%=check[j] %>" >  <%=check[j] %>
			                </td>
			                <td align="center"><input type="text" id="INFO1" name="INFO1" class="input-text lh30" size="30"></td>
			                <td ><input id="spinner<%=k %>" value=5 name="INFO_LEVEL1" size=8 readonly></td>
			                </tr>
                             <%
                             k++;
                             }
                ...
                <c:choose>    
                <c:when test="${not empty policyList2}">
                    <% String[] a = {"车主生日","房屋结构类型","车主姓名","车主证件号码","被保房屋用途","邮政编码","银行名称","是否邮寄保单","车牌号","是否邮寄发票","第三方支付公司名称","房屋详细地址","车主性别","借记卡/信用卡号","保障类型","网银帐户名","第三方支付帐号名","邮寄地址","邮寄邮编"};
                        String[] b = new String[a.length];
                        int i=0;
                      %>
                     <c:forEach var="policyList" items="${policyList2}">
                        <c:set var="check2" value="${policyList.POLICY_INFO}" scope="request"/> 
	                       <tr>
			                <td>&nbsp;</td>
			                <td>
			                  <input type="checkbox" name="POLICY_INFO2" checked value="${policyList.POLICY_INFO}" >  ${policyList.POLICY_INFO}
			                </td>
			                <td align="center"><input type="text" value="${policyList.INFO}" id="INFO" name="INFO2" class="input-text lh30" size="30"></td>
			                <td ><input id="spinner<%=k %>" value="${policyList.INFO_LEVEL}" name="INFO_LEVEL2" size=8 readonly></td>
			               </tr>
			               
			           <% b[i] = request.getAttribute("check2").toString();
			              i++;
			              k++;
			           %>
                     </c:forEach>
                      ...
                      if(null != check || check.length == 0){
                          for(int j=0;j<check.length;j++){
                           if(check[j] != null){
			                ...
			                <td>&nbsp;</td>
			                <td>
			                  <input type="checkbox" name="POLICY_INFO2" value="<%=check[j] %>" >  <%=check[j] %>
			                </td>
			                <td align="center"><input type="text" id="INFO" name="INFO2" class="input-text lh30" size="30"></td>
			                <td ><input id="spinner<%=k %>" value=5 name="INFO_LEVEL2" size=8 readonly></td>
			                </tr>
                             <%
                             k++;
                             }
                ...
                <td>&nbsp;</td>
                <td>
                  <input type="checkbox" name="POLICY_INFO2" value="邮寄邮编">  邮寄邮编
                </td>
                <td align="center"><input type="text" id="INFO2" name="INFO2" class="input-text lh30" size="30"></td>
