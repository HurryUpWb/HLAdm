...
function loadMuOrderList() {
	...
	document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/adminOrderSearch/MuIndex.json?currentFunc=1\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
function loadBbgOrderList() {
	// 当前位置设定
	$("#current_func").text("北部湾订单查询");
	document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/adminOrderSearch/MuIndex.json?currentFunc=2\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
function loadWestOrderList() {
	// 当前位置设定
	$("#current_func").text("西部航空订单查询");
	document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/adminOrderSearch/WestIndex.json?currentFunc=3\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
/**
 * 加载ATM订单查询
 */
...
function CompanyInfoConfig() {
    // 当前位置设定
    $("#current_func").text("航司信息配置");
    document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/QRCode/CompanyInfoConfig.json\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
...
function empQRcode() {
    // 当前位置设定
    $("#current_func").text("机场二维码");
    document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/QRCode/index.json\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}

/**
 * 机场二维码生成
 */
function QRcodeSalesStatics() {
    // 当前位置设定
    $("#current_func").text("二维码销售统计");
    document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/QRCode/getStatistics.json\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
...
function loadCancelOrder() {
	...
	document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/adminOrderSearch/CancelOrder.json?CancelOrderId=1\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
function bbgLoadCancelOrder() {
	// 当前位置设定
	$("#current_func").text("北部湾退保退款");
	document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/admin/adminOrderSearch/CancelOrder.json?CancelOrderId=2\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
/**
 * H5退保退款
 */
function loadCancelH5Order(){
	$("#current_func").text("H5退保退款");
	//admin/adminOrderSearch/CancelH5Order.json
	document.getElementById("main").innerHTML="<iframe name=\"right\" id=\"rightMain\" src=\"" + getLocationUrl()+"/order/cancelH5Order.jsp\" frameborder=\"no\" scrolling=\"auto\" width=\"100%\" height=\"auto\" allowtransparency=\"true\"></iframe>";
}
	...
	callSvr("adminOrderSearch/OrderSearch",$("#orderSearch").formToJson(),function(data) {
		if(null != data.err && data.err == 9){
			Dialog.alert("提示：您没有登录或登录已超时！",function(){parent.location.href = "/Admin/login.html";});
				...
				table.append("<th width='5%'>保费</th><th width='7%'>下单时间</th><th width='8%'>订单来源</th><th width='8%'>查看详情</th><th width='8%'>状态</th></tr>");
				if (data != null && typeof (data) != "undefined") {
					...
					for (var i = 0; i < data.length; i++) {
						...
						if(1 == data[i].ORDER_TYPE) {
							...
							// 东航H5端
							table.append("东航H5端");
						}else if(7 == data[i].ORDER_TYPE) {
							// 北部湾
							table.append("北部湾");
						}else if(8==data[i].ORDER_TYPE){
							//批量投保
							table.append("批量投保");
						}else if(9 == data[i].ORDER_TYPE) {
							// 北部湾
							table.append("积友之家");
						}else if(10 == data[i].ORDER_TYPE) {
							// 北部湾
							table.append("西部航空");
						}
						table.append("</td><tD>");
						table.append("<input type=\"button\" onclick=\"CheckDetail('"+data[i].TRANS_NO+"')\" class=\"ext_btn ext_btn_success\" value=\"订单详情\">");
						table.append("</td><tD>");
						if(data[i].POLICY_NO !=""){
							if(data[i].STATE == 140){
								table.append("已退款");
							}else if(data[i].STATE == 150){
								table.append("已退保");
							}else{
								table.append("已承保");
							}
							...
							if(data[i].STATE == 110){
								...
								table.append("未支付");
							}else if(data[i].STATE == 120){
								table.append("投保失败");
							}else if(data[i].STATE == 130){
								table.append("已承保");
							}else if(data[i].STATE == 140){
								table.append("已退款");
							}else if(data[i].STATE == 150){
								table.append("已退保");
							}else{
...
function muOrderInquiry(currentFunc){
	var muForm = document.getElementById("muSearch");
	muForm.action = "admin/adminOrderSearch/MuSearch.json?currentFunc="+currentFunc+"";
	muForm.submit();
}

function westOrderInquiry(currentFunc){
	var muForm = document.getElementById("westSearch");
	muForm.action = "admin/adminOrderSearch/WestSearch.json?currentFunc="+currentFunc+"";
	muForm.submit();
}
 ...
 * 退款
 */
function drawback(transno){
	callSvr("MUOrderHandle/refund",{"orderNo":transno},function(data) {
		if(null != data.err){
			if(data.err == 9){
				Dialog.alert("提示：您没有登录或登录已超时！",function(){parent.location.href = "/Admin/login.html";});
			}else if(data.err == 0){
 ...
 * H5 退款
 */
function H5drawback(transno){
	callSvr("MUOrderHandle/refund",{"orderNo":transno},function(data) {
		if(null != data.err){
			if(data.err == 9){
				Dialog.alert("提示：您没有登录或登录已超时！",function(){parent.location.href = "/Admin/login.html";});
			}else if(data.err == 0){
				Dialog.alert("提示：退款请求发送成功！");
			}else if(data.err == 1){
				Dialog.alert("提示：退款的订单数据不存在！");
			}else if(data.err == 2){
				Dialog.alert("提示：退款失败，退款请求异常！");
 ...
 * 退保，根据流水号退保
 */
function surrender(transno){
	callSvr("MUOrderHandle/surrender",{"orderNo":transno},function(data) {
		if(null != data.err){
			if(data.err == 9){
				Dialog.alert("提示：您没有登录或登录已超时！",function(){parent.location.href = "/Admin/login.html";});
			}else if(data.err == 0){
 ...
 * H5端 退保，根据流水号退保
 */
function H5surrender(transno){
	callSvr("MUOrderHandle/surrender",{"orderNo":transno},function(data) {
		if(null != data.err){
			if(data.err == 9){
				Dialog.alert("提示：您没有登录或登录已超时！",function(){parent.location.href = "/Admin/login.html";});
			}else if(data.err == 0){
				Dialog.alert("提示：退保成功！");
			}else if(data.err == 7){
				Dialog.alert("提示：退保请求发送成功！");
			}else if(data.err == 1){
				Dialog.alert("提示：退保的订单数据不存在！");
	...
	table.append("<th width='5%'>保费</th><th width='7%'>下单时间</th><th width='8%'>订单来源</th><th width='8%'>查看详情</th><th width='8%'>状态</th></tr>");
		for (var i = page*10; i < billData.length; i++) {
			...
			if(1 == billData[i].ORDER_TYPE) {
				...
				// 东航H5端
				table.append("东航H5端");
			}else if(8 == billData[i].ORDER_TYPE){
				//批量投保
				table.append("批量投保");
			}else if(7 == data[i].ORDER_TYPE) {
				// 北部湾
			...
			table.append("</td><tD>");
			table.append("<input type=\"button\" onclick=\"CheckDetail('"+billData[i].TRANS_NO+"')\" class=\"ext_btn ext_btn_success\" value=\"订单详情\">");
			table.append("</td><tD>");
 ...
 * @constructor
 */
function CheckDetail(transno){
	$.ajax({
		url:getLocationUrl()+"/admin/adminOrderSearch/OrderDetail.json",
		cache:false,
		async:false,
		data:{'number':transno},
		dataType:'json',
		success:function (data) {
			...
			$("#InsurancePolicyDetail").append(
				...
				'<tr><td>投保失败原因</td><td>'+NullKiller(data.INSURANCE_FAILED_REASON)+'</td></tr>'+
				...
				'<tr><td>投保时间</td><td>'+NullKiller(data.INSURANCE_TIME)+'</td></tr>'+
				'<tr><td>投保失败时间</td><td>'+NullKiller(data.INSURED_FAILURE_TIME)+'</td></tr>'+
				'<tr><td>退保时间</td><td>'+NullKiller(data.SURRENDER_TIME)+'</td></tr>'+
				'<tr><td>退款时间</td><td>'+NullKiller(data.REFUND_TIME)+'</td></tr>'+
 ...
 * @constructor
 */
function CheckTheDetail(transno){
	$.ajax({
...
/**
 * 西部航空查询订单详情，根据订单流水号查询
 * @param number
 * @constructor
 */
function CheckWestDetail(transno){
	$.ajax({
		url:getLocationUrl()+"/admin/adminOrderSearch/WestOrderDetailed.json",
		cache:false,
		async:false,
		data:{'number':transno},
		dataType:'json',
