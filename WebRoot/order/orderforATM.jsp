 ...
 <c:set var="pageList" scope="page" value="${list}"/>
	<div class="container">
		...
		<div id="search_bar" class="mt10">
			<form id="muSearch" name="muSearch" method="post" action="admin/adminOrderSearch/ATMOrderSearch.json">
				<!-- 查询条件 -->
				<table class="search_table" width="100%" border="0" cellpadding="0"
					...
					<tr>
						...
						<option <c:if test="${BILL_STATUS =='' }">selected</c:if> value="">全部</option>
						<option <c:if test="${BILL_STATUS =='100' }">selected</c:if> value="100">未支付</option>
						<option <c:if test="${BILL_STATUS =='110' }">selected</c:if> value="110">已支付</option>
						<option <c:if test="${BILL_STATUS =='120' }">selected</c:if> value="120">承保失败</option>
						<option <c:if test="${BILL_STATUS =='130' }">selected</c:if> value="130">已承保</option>
						<option <c:if test="${BILL_STATUS =='150' }">selected</c:if> value="150">已退保</option>
						<option <c:if test="${BILL_STATUS =='140' }">selected</c:if> value="140">已退款</option>
						</select></td>
					</tr>
					<tr>
						<td>手机号：</td>
					...
					<tr>
						...
						<option <c:if test="${ORDER_TYPE =='3' }">selected</c:if> value="3">ATM机</option>
						</select>
						</td>
					</tr>
					<tr>
				...
				<div id="button" class="mt10"><input type="button"
							onclick="ATMOrderInquiry();" name="searchBtn"
							class="btn btn82 btn_search" value="查询"> &nbsp;&nbsp;<input
							type="button" onclick="AutomateExcel('11');" name="exportBtn"
						...
						<c:if test="${not empty pageList.list }">
						      <c:forEach var="list" items="${pageList.list }">
											<tr>
												...
												<th width="8%"><c:if test="${list.ORDER_TYPE=='5' }">PC端</c:if><c:if test="${list.ORDER_TYPE=='6' }">H5端</c:if></th>
												<th width="8%">
												...
												<th width="6%"><input type="button" onclick="CheckTheATMDetail('${list.ORDER_NUMBER}')" class="ext_btn ext_btn_success" value="订单详情"></th>
												<th width="6%">
												</th>
											</tr>
								</c:forEach>
			...
			<c:if test="${pageList.totalRow>0}">
              <div class="page mt10">
                <div class="pagination">
				            ...
				            <c:set var="formName" scope="request" value="ATMOrderSearch" />
				            <c:set var="pageUrl" scope="request" value="admin/adminOrderSearch/ATMOrderSearch.json?pageIndex=" />
		...
		<div id="HiddenInsurancePolicyDetail" style="position: fixed;width: 100%;height: 100%;top: 0;left: 0;z-index: 1000;background:white;display: none;overflow: auto">
